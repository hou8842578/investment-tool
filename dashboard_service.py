...
def build_partner_summary(records):
    ...
    for record in records:
        ...
        if record['returned'] and record.get('finalInterest'):
            bucket['interest'] += round((record.get('finalInterest') or 0) * partner_amount / record['amount'])
    ...
    for name in sorted(partner_map.keys()):
        ...
        item['yieldRate'] = round(item['interest'] / item['amount'] * 100, 2) if item['amount'] > 0 else 0
        result.append(item)
    return result
...
def build_monthly_interest(records):
    ...
    for record in records:
        ...
        if not month:
            continue
        month_map[month] = month_map.get(month, 0) + (record.get('finalInterest') or 0)
    return [{'month': month, 'value': month_map[month]} for month in sorted(month_map.keys())]
